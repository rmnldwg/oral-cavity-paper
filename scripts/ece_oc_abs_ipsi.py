...
a = plt.bar(POSITIONS, ece_ipsi, label= "ECE ipsi (" + str(int(sum(ece_ipsi)/2)) + ")", color="#ae0060", width=WIDTHS)
b = plt.bar(POSITIONS, noece_ipsi, bottom=ece_ipsi, label="no ECE ipsi ("  + str(int(sum(noece_ipsi)/2)) + ")", color="#00afa5", width=WIDTHS)
c = plt.bar(POSITIONS, ece_ipsi_unk, bottom=ece_ipsi + noece_ipsi, label="unknown ("  + str(int(sum(ece_ipsi_unk)/2)) + ")", color="#c5d5db", width=WIDTHS)
