...
a = plt.bar(POSITIONS, ece_contra/sums_contra*100, label= "ECE contra (" + str(int(sum(ece_contra)/2)) + ")", color="#ae0060", width=WIDTHS)
b = plt.bar(POSITIONS, noece_contra/sums_contra*100, bottom=ece_contra/sums_contra*100, label="no ECE contra ("  + str(int(sum(noece_contra)/2)) + ")", color="#00afa5", width=WIDTHS)
c = plt.bar(POSITIONS, ece_contra_unk/sums_ipsi*100, bottom=(ece_contra + noece_contra)/sums_contra*100, label="unknown ("  + str(int(sum(ece_contra_unk)/2)) + ")", color="#c5d5db", width=WIDTHS)

i = 0
for p in a.patches:
    if p.get_height()/2 > 3:
        plt.annotate(format(p.get_height()/100*sums_contra[i], '.0f'), 
...
j = 0
for p in b.patches:
    plt.annotate(format(p.get_height()/100*sums_contra[j], '.0f'), 
                 (p.get_x() + p.get_width() / 2., p.get_height()/2 +ece_contra[j]/sums_contra[j]*100), 
                   ha = 'center', va = 'center', 
                   size=7,
                   xytext = (0, 0), 
                   textcoords = 'offset points')
...
for p in c.patches:
    if p.get_height() > 3:
        plt.annotate(format(p.get_height()/100*sums_contra[k], '.0f'), 
                    (p.get_x() + p.get_width() / 2., 100 - 100*ece_contra_unk[k]/sums_contra[k]/2), 
                    ha = 'center', va = 'center', 
                    size=7,
                    xytext = (0, 0), 
                    textcoords = 'offset points')
