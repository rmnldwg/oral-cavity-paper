...
b = plt.bar(
    ...
    noece / sums * 100,
    bottom=ece / sums * 100,
    label="no ECE (" + str(int(sum(noece) / 2)) + ")",
    color="#00afa5",
    width=WIDTHS,
)
c = plt.bar(
    POSITIONS,
    ece_unk / sums * 100,
    bottom=(ece + noece) / sums * 100,
    label="unknown (" + str(int(sum(ece_unk) / 2)) + ")",
    color="#c5d5db",
    width=WIDTHS,
)
...
for p in a.patches:
    plt.annotate(
        format(p.get_height() / 100 * sums[i], ".0f"),
...
for p in b.patches:
    plt.annotate(
        format(p.get_height() / 100 * sums[j], ".0f"),
        (p.get_x() + p.get_width() / 2.0, p.get_height() / 2 + ece[j] / sums[j] * 100),
        ha="center",
        va="center",
        size=7,
        xytext=(0, 0),
...
for p in c.patches:
    if p.get_height() > 3:
        plt.annotate(
            format(p.get_height() / 100 * sums[k], ".0f"),
            (p.get_x() + p.get_width() / 2.0, 100 - 100 * ece_unk[k] / sums[k] / 2),
            ha="center",
            va="center",
            size=7,
            xytext=(0, 0),
