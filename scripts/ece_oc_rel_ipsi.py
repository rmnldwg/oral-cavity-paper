...
a = plt.bar(
    POSITIONS,
    ece_ipsi / sums_ipsi * 100,
    label="ECE ipsi (" + str(int(sum(ece_ipsi) / 2)) + ")",
    color="#ae0060",
    width=WIDTHS,
)
b = plt.bar(
    POSITIONS,
    noece_ipsi / sums_ipsi * 100,
    bottom=ece_ipsi / sums_ipsi * 100,
    label="no ECE ipsi (" + str(int(sum(noece_ipsi) / 2)) + ")",
    color="#00afa5",
    width=WIDTHS,
)
c = plt.bar(
    POSITIONS,
    ece_ipsi_unk / sums_ipsi * 100,
    bottom=(ece_ipsi + noece_ipsi) / sums_ipsi * 100,
    label="unknown (" + str(int(sum(ece_ipsi_unk) / 2)) + ")",
    color="#c5d5db",
    width=WIDTHS,
)
...
for p in a.patches:
    if p.get_height() > 3:
        plt.annotate(
            format(p.get_height() / 100 * sums_ipsi[i], ".0f"),
...
for p in b.patches:
    plt.annotate(
        format(p.get_height() / 100 * sums_ipsi[j], ".0f"),
        (
            p.get_x() + p.get_width() / 2.0,
            p.get_height() / 2 + ece_ipsi[j] / sums_ipsi[j] * 100,
        ),
        ha="center",
        va="center",
        size=7,
...
for p in c.patches:
    if p.get_height() > 3:
        plt.annotate(
            format(p.get_height() / 100 * sums_ipsi[k], ".0f"),
            (
                p.get_x() + p.get_width() / 2.0,
                100 - 100 * ece_ipsi_unk[k] / sums_ipsi[k] / 2,
            ),
            ha="center",
            va="center",
            size=7,
